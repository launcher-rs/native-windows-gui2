...
impl FileDialog {
    ...
    pub fn run<C: Into<ControlHandle>>(&self, parent: Option<C>) -> bool {
        ...
        let parent_handle = match parent {
            Some(p) => p
                .into()
                .hwnd()
                .expect("File dialog parent must be a window control"),
            None => ptr::null_mut(),
        };
    ...
    pub fn get_selected_item(&self) -> Result<OsString, NwgError> {
        if self.multiselect() {
            return Err(NwgError::file_dialog(
                "FileDialog have the multiselect flag",
            ));
        }

        unsafe { rh::filedialog_get_item(&mut *self.handle) }
    }
    ...
    */
    pub fn get_selected_items(&self) -> Result<Vec<OsString>, NwgError> {
        if self.action == FileDialogAction::Save {
            return Err(NwgError::file_dialog(
                "Save dialog cannot have more than one item selected",
            ));
        }

        unsafe { rh::filedialog_get_items(&mut *(self.handle as *mut IFileOpenDialog)) }
    }
    ...
    */
    pub fn set_multiselect(&self, multiselect: bool) -> Result<(), NwgError> {
        ...
        if self.action == FileDialogAction::Save {
            return Err(NwgError::file_dialog(
                "Cannot set multiselect flag for a save file dialog",
            ));
        }

        let result = unsafe {
            rh::toggle_dialog_flags(&mut *self.handle, FOS_ALLOWMULTISELECT, multiselect)
        };
        match result {
            Ok(_) => Ok(()),
            Err(e) => Err(e),
