...
impl ImageList {
    ...
    pub fn size(&self) -> (i32, i32) {
        ...
        let mut size = (0, 0);
        unsafe {
            ImageList_GetIconSize(self.handle, &mut size.0, &mut size.1);
        }
    ...
    pub fn set_size(&self, size: (i32, i32)) {
        ...
        let (w, h) = size;
        unsafe {
            ImageList_SetIconSize(self.handle, w, h);
        }
    }
    ...
    */
    pub fn add_bitmap_from_filename(&self, filename: &str) -> Result<i32, NwgError> {
        ...
        unsafe {
            Ok(ImageList_AddMasked(
                self.handle,
                bitmap.handle as HBITMAP,
                0,
            ))
        }
    }
    ...
    */
    pub fn remove(&self, index: i32) {
        ...
        unsafe {
            ImageList_Remove(self.handle, index);
        }
    }
    ...
    pub fn replace_icon(&self, index: i32, icon: &Icon) {
        ...
        unsafe {
            ImageList_ReplaceIcon(self.handle, index, icon.handle as HICON);
        }
    }
}
